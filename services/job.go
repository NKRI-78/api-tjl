...
func JobStore(j *models.JobStore) (map[string]any, error) {
	...
	query := `INSERT INTO jobs (uid, title, caption, salary, cat_id, place_id, is_draft) 
	VALUES (?, ?, ?, ?, ?, ?, ?)`

	err := db.Debug().Exec(query, j.Id, j.Title, j.Caption, j.Salary, j.CatId, j.PlaceId, j.IsDraft).Error
